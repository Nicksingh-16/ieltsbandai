...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-surface-50">Create Question</h1>
    ...
    <form method="POST" action="{{ route('admin.questions.store') }}" class="card p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Describe the chart below…"
                    class="w-full bg-surface-900 border border-surface-700 rounded-lg px-3 py-2.5 text-surface-100 text-sm focus:outline-none focus:border-indigo-500">
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm text-surface-400 mb-1">Content / Full Question <span class="text-red-400">*</span></label>
                <textarea name="content" rows="6" required placeholder="Full question text, passage, or instructions…"
            ...
            <div>
                <label class="block text-sm text-surface-400 mb-1">Media URL <span class="text-surface-600 text-xs">(optional, for audio/image)</span></label>
            ...
            <div>
                <label class="block text-sm text-surface-400 mb-1">Time Limit <span class="text-surface-600 text-xs">(seconds)</span></label>
                <input type="number" name="time_limit" value="{{ old('time_limit') }}" min="1" placeholder="e.g. 2400"
                    class="w-full bg-surface-900 border border-surface-700 rounded-lg px-3 py-2.5 text-surface-100 text-sm focus:outline-none focus:border-indigo-500">
            </div>

            <div>
                <label class="block text-sm text-surface-400 mb-1">Min Words <span class="text-surface-600 text-xs">(for writing)</span></label>
                <input type="number" name="min_words" value="{{ old('min_words') }}" min="1" placeholder="e.g. 150"
                    class="w-full bg-surface-900 border border-surface-700 rounded-lg px-3 py-2.5 text-surface-100 text-sm focus:outline-none focus:border-indigo-500">
            </div>
        </div>
