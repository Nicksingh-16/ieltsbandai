...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <a href="{{ route('institute.question-sets.index') }}" class="text-surface-500 hover:text-surface-300 text-sm">← Question Sets</a>
            <h1 class="text-2xl font-bold text-surface-50 mt-2">{{ $set->name }}</h1>
        ...
        <a href="{{ route('institute.questions.create') }}" class="btn-secondary text-sm">+ New Question</a>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-3">
            ...
            <div class="card overflow-hidden">
                ...
                <div class="flex items-start gap-4 px-4 py-3 border-b border-surface-800 last:border-0">
                    ...
                    @if(Auth::user()->isTeacher())
                    <form method="POST" action="{{ route('institute.question-sets.remove-question', [$set, $q]) }}"
        ...
        <div class="lg:col-span-2">
            ...
            <div class="card p-4">
                ...
                <form method="POST" action="{{ route('institute.question-sets.add-question', $set) }}" class="flex gap-2 mb-4">
                    @csrf
                ...
                <p class="text-surface-500 text-xs">
                    {{ $availableQuestions->count() }} questions available from your bank.
                </p>
                @else
                <p class="text-surface-500 text-sm">
                    No more questions to add.
                    <a href="{{ route('institute.questions.create') }}" class="text-indigo-400">Create questions →</a>
                </p>
                @endif
            </div>
        </div>
