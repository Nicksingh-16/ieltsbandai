...
<div class="min-h-screen bg-surface-950 py-10 px-4">
    <div class="max-w-4xl mx-auto">
        ...
        {{-- Mock test continuation — auto-advance after countdown (see listening/result.blade.php for pattern). --}}
        @if(session('mock_test_id'))
        <div class="card p-5 border border-brand-500/30 bg-brand-500/5 mb-4">
            <p class="font-semibold text-brand-400 text-sm mb-3">✅ Reading complete — continuing to Writing in <span id="mockCountdown">4</span>s</p>
            <form id="mockAdvanceForm" method="POST" action="{{ route('mock-test.advance', ['mock' => session('mock_test_id'), 'module' => 'reading']) }}">
                @csrf
